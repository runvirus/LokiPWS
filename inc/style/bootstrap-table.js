        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
					...
					if(field == "data") //Long text
					{
						var OutValue;
						var mutex__ = '';
						if(value.length > 80)
						{
							var n = value.indexOf("datadl");
							if(n == -1)
							{
								OutValue = value.substr(0, 76) + " ...";
								mutex__ = 'sho___aa';
							}
							else
								OutValue = value;
						...
						text = that.options.cardView ?
							['<div class="card-view">',
								...
								sprintf('<span class="value">%s</span>', OutValue),
								'</div>'].join('') :
							[sprintf('<td%s id="%s" name=\'%s\' %s %s>', id_, mutex__, value, class_, style),
								OutValue,
								'</td>'].join('');
					}
					else
